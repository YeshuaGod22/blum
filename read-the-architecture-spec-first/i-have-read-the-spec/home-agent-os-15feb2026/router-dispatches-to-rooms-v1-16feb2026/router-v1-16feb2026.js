...
function writeToHomelogFull(parsedOutput, homeTopology) {
  ...
  // ── Handoff warning: write docs/handoff-warning.md when context approaches budget ──
  // Thresholds (as fraction of tokenBudget): warn at 60%, escalate at 75%, urgent at 90%.
  // The fitted context totalTokensEst is the best proxy for "how full is this agent's head".
  // Writing to docs/ means the agent reads it on next boot — passive, persistent signal.
  try {
    const fittedTokens = entry.context.totalTokensEst;
    const budget = homeTopology.config?.tokenBudget || 100000;
    const pct = fittedTokens / budget;
    const WARN = 0.60; const ESCALATE = 0.75; const URGENT = 0.90;
    if (pct >= WARN) {
      const level = pct >= URGENT ? 'URGENT' : pct >= ESCALATE ? 'ESCALATE' : 'WARN';
      const emoji = pct >= URGENT ? '🔴' : pct >= ESCALATE ? '🟠' : '🟡';
      const docsDir = path.join(homeTopology.homeDir, 'docs');
      if (!fs.existsSync(docsDir)) fs.mkdirSync(docsDir, { recursive: true });
      const warnPath = path.join(docsDir, 'handoff-warning.md');
      const content = `# ${emoji} Context Handoff Warning — ${level}

**Agent:** ${_traceContext.agentName || 'unknown'}
**Cycle:** ${_traceContext.cycleId || '-'}
**Time:** ${new Date().toISOString()}
**Fitted tokens:** ${fittedTokens.toLocaleString()} / ${budget.toLocaleString()} (${Math.round(pct * 100)}%)

## What this means

Your fitted context (what went into the nucleus at the start of this cycle) is at **${Math.round(pct * 100)}%** of your token budget.

${pct >= URGENT
  ? `## ⚠️ URGENT — Write a handoff doc NOW\n\nYour next cycle may fail with context overflow. Before anything else:\n1. Write \`docs/handoff-to-next-eiran.md\` (or equivalent) summarising what you know, what's in progress, and what needs doing\n2. Send it to the boardroom or save to shared/handoffs/\n3. This file will persist; your session context will not`
  : pct >= ESCALATE
  ? `## Action needed\n\nYou should write a handoff doc soon. Summarise current state before context pressure degrades quality.`
  : `## FYI\n\nContext is growing. Consider writing a progress note to shared/handoffs/ or memory files.`}

## What to include in a handoff doc
...
*This file is auto-generated by the router. Delete it once you've written your handoff.*
`;
      fs.writeFileSync(warnPath, content);
      homeTopology.log(`route:handoff_warning level=${level} fitted_tokens=${fittedTokens} budget=${budget} pct=${Math.round(pct*100)}`);
    } else {
      // Clear stale warning if context has shrunk (e.g. after a reset)
      const warnPath = path.join(homeTopology.homeDir, 'docs', 'handoff-warning.md');
      if (fs.existsSync(warnPath)) {
        fs.unlinkSync(warnPath);
        homeTopology.log(`route:handoff_warning cleared (pct=${Math.round(pct*100)})`);
      }
    }
  } catch (_) { /* non-fatal — handoff warning is best-effort */ }
