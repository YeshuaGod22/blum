...
**Date:** 11 Sep 2026  
**Status:** experimental UI + runner-core + retrospective-import + corpus-explorer + battery-library module  
...
- `blum-experimental-lineage-lab-entrance-11sep2026.html` — the four-door lab entrance: Design, Rehearse, Import, Explore.
...
5. **A flown/frozen trunk is immutable.** Editing a frozen design creates a new manifest version rather than silently changing history.
6. **Failure states are observations.** Refusal, parse failure, API failure, truncation, context limit, protocol stop, operator abort, and missing XML tag are explicit outcomes.
7. **Analysis streams are preregistrable.** XML-region selectors and missing-tag policies are part of the frozen manifest.
8. **The design exposes its geometry before execution.** Call counts, paired contrasts, factorial cells, battery coverage, and obvious missing cells are visible before or after a run as appropriate.
9. **Complexity is folded, not removed.** The default workflow is Build → Fork → Probe → Measure → Review → Freeze; advanced details remain available without forcing them into the operator's working memory.
10. **Raw output and analysis projection are different objects.** The runner preserves raw model output; XML selectors create derived projections later.
11. **Exact means exact.** An `a ↔ 0` pair receives `exact_shared_parent` only when both observations carry the same real `parentSnapshotId`.
12. **Historical labels do not prove ancestry.** For DAE raw-call records, `parent_prefix` and branch names are corroborating metadata only; exact ancestry is computed from the first `prefix_len` messages in the archived `sent` array.
13. **Call integrity and section integrity are separate.** A `max_tokens` response can contain clean completed sections while another section is damaged; those facts are preserved independently.
14. **Corpus visualization is derived, not canonical.** The tree, coverage matrix, and anomaly views can always be rebuilt from the lineage dataset.
...
`experiment → trunks → parent snapshots → forks → battery attachment → probes → observations → analysis projections`, with every frozen design and battery assigned fingerprints and every exact sibling comparison anchored to an immutable parent snapshot.
