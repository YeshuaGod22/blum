...
async function main() {
  ...
  const witnessParents = JSON.parse(await fs.readFile(path.join(out, 'data', 'dae-witness-parent-snapshot-integrity-v0.json'), 'utf8'));
  ...
  assert.equal(index.portableProjection.witnessParentSnapshotIntegrityStandalone, true);
  ...
  assert.equal(Object.hasOwn(index, 'witnessParentSnapshotIntegrity'), false, 'whole index must not duplicate witness-parent proof rows');
  ...
  // Frozen branch-witness parent snapshots remain inspectable offline. This
  // layer covers every raw2+ branch call; only 266 need it as fallback because
  // 1442 also have a materialized parent trajectory.
  assert.equal(witnessParents.schema, 'blum-dae-witness-parent-snapshot-integrity-v0');
  assert.equal(witnessParents.branchCallCount, 1706);
  assert.equal(witnessParents.verifiedCount, 1706);
  assert.deepEqual(witnessParents.statuses, { verified_witness_parent_snapshot_extension: 1706 });

  // Combined topology proof survives offline with zero unverified branches.
  ...
  assert.deepEqual(integrity.branchParentRelationStatuses, {
    verified_exact_extension: 1442,
    verified_witness_parent_snapshot_extension: 266,
  });
  assert.deepEqual(integrity.branchParentRelationProofClasses, {
  ...
  assert.equal(integrity.parentRelations.filter(x => x.proofClass === 'unverified').length, 0);
  ...
  assert.equal(incomplete[0].incompleteCalls[0].turn, 5);
  assert.equal(incomplete[0].incompleteCalls[0].stopReason, 'max_tokens');
  ...
  const branches = instances.filter(x => x.instanceKind === 'branch_from_lived_trunk');
  assert.equal(branches.length, 1708);
  assert.ok(branches.every(x => ['materialized_parent_trajectory','frozen_witness_parent_snapshot'].includes(x.parentRelationProofClass)));
  ...
  assert.equal(manifest.witnessParentSnapshotIntegrity.branchCallCount, 1706);
  assert.equal(manifest.witnessParentSnapshotIntegrity.verifiedCount, 1706);
  assert.equal(manifest.trajectoryPackageIntegrity.materializedParentVerifiedCount, 1442);
  assert.equal(manifest.trajectoryPackageIntegrity.witnessSnapshotParentVerifiedCount, 266);
  assert.equal(manifest.trajectoryPackageIntegrity.unverifiedParentCount, 0);
  ...
  for (const required of [
    ...
    'app/dae-witness-parent-snapshot-integrity-v0-15sep2026.js',
  ...
  console.log(JSON.stringify({
    ...
    topology: {
      ...
      proofClasses: integrity.branchParentRelationProofClasses,
      unverified: integrity.unverifiedParentCount,
    },
    witnessParentSnapshots: witnessParents.statuses,
