...
async function main() {
  ...
  assert.equal(index.inferencePackageGraphRef.path, 'data/dae-inference-package-graph-v0.json');
  ...
  // Compact means event nodes carry hashes/refs, while text lives once in the
  // content-addressed store. No package should embed another full package copy.
  ...
  // Compatibility observation/message projection remains self-contained too.
