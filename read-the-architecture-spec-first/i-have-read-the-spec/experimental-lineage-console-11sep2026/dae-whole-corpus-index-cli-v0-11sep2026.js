...
// Usage:
//   node dae-whole-corpus-index-cli-v0-11sep2026.js <EXP-003-root> [output.json]
//
// Builds one battery-observation index across Pilot 1 + every rawN collection
// present under the experiment root. Pilot 1 uses the validated record.json
// derivation; raw2+ uses the existing raw-call retrospective importer.

const fs = require('fs');
const path = require('path');
const Importer = require('./dae-corpus-lineage-import-cli-v0-11sep2026.js');
const Unified = require('./dae-unified-observation-index-v0-11sep2026.js');
...
function main(argv) {
  ...
  const output = argv[3] || 'dae-whole-corpus-index.json';
  if (!experimentRoot) {
    console.error('Usage: node dae-whole-corpus-index-cli-v0-11sep2026.js <EXP-003-root> [output.json]');
    process.exitCode = 2;
    return;
  }
