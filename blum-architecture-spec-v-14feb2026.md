...
BootAssembler {
  ...
  //   Each document carries _meta: { docId, source, file, slot, cycleId }
  // tools[] is capability — tool definitions from the home's tools/
  //   directory. Each tool carries _meta: { file, cycleId }
  //   The home passes tools to the nucleus alongside messages.
  //   Tools are NOT context. They are capabilities the home grants.
  //   _meta is stripped before API calls (see section 12).
}
...
ContextManager {
  ...
  //
  // Each message carries _meta with source provenance
  // (boot:identity, context:room-info, history:{room}).
  // _meta is stripped before API calls (see section 12).
  //
...
OutputProcessor {
  ...
  // thinking: Array<{ blockId, content }> — each block has a UID
  // messages: Array<{ blockId, to, content }> — each block has a UID
...
## 12. Traceability — Every Datum Has a UID

Every piece of data that flows through a processing cycle carries metadata
and a unique identifier, so the full chain from input to output is traceable.

### The `_meta` convention

Metadata fields are prefixed with `_` (underscore). Every message, document,
and tool definition in the processing pipeline carries a `_meta` object with
provenance information. **`_meta` fields are NEVER sent to the LLM.** The
home strips them before every `nucleus.call()` via `cleanMessages()` and
`cleanTools()`.
...
Room server IDs (kept as-is): `msg`, `op`, `uid`, `room`.

All UIDs follow the format `prefix_16hexchars` (e.g. `cycle_a3f8b2c1d4e5f6a7`),
generated by the shared utility at `shared-uid-generator/generate-uid.js`.

### The trace chain

A single processing cycle produces this chain:

```
dispatch (dispatchId) arrives from room server
  → home creates cycle (cycleId)
    → boot assembler tags documents (docId per document)
    → context manager tags messages (ctxId per message, source provenance)
...
### `_traceContext` threading

The home creates a `_traceContext` object at the start of each processing
cycle and threads it through every module:

```javascript
const _traceContext = {
  cycleId,          // unique to this cycle
  dispatchId,       // from the room server's dispatch
  room,             // triggering room name
  agentName,        // this agent's name
  startedAt,        // ISO timestamp
  iterations: [],   // accumulated tool loop iterations
};
```
...
BootAssembler {
  assemble(agentConfig, history, _traceContext?) → { documents[], tools[] }
  // Each document gets _meta: { docId, source, file, slot, cycleId }
  // Each tool gets _meta: { file, cycleId }
}

ContextManager {
  build(home, dispatch, bootDocuments, tokenBudget, _traceContext?) → messages[]
  // Boot docs pass through with existing _meta
  // Room context gets _meta: { ctxId, source: 'context:room-info', ... }
  // History msgs get _meta: { ctxId, source: 'history:{room}', originalMsgId, ... }
}

OutputProcessor {
  parse(responseText, _traceContext?) → { parseId, thinking[], messages[], private, _meta }
  // thinking: Array<{ blockId, content }>
  // messages: Array<{ blockId, to, content }>
}
...
## 13. Future Compatibility — Don't Close These Doors
<!-- (was section 12 before traceability section was added) -->
...
- [ ] Does my new data carry `_meta` with a UID and source provenance? (See section 12)
- [ ] Am I stripping `_meta` before sending anything to the nucleus?
